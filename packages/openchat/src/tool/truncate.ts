...
const TRUNCATION_DIR = path.join(Global.Path.data, 'tool-output');

export const MAX_LINES = 2000;
export const MAX_BYTES = 50 * 1024;
export const GLOB = path.join(TRUNCATION_DIR, '*');

const hasTaskTool = (agent?: SchemaAgent.Info) => {
  if (!agent?.permission) {
    return false;
  }
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const write = Effect.fn('Truncate.write')(function* (text: string) {
      const file = path.join(TRUNCATION_DIR, SchemaTool.ToolID.ascending());
    ...
    const limits = Effect.fn('Truncate.limits')(function* () {
      ...
      const cfg = yield* configSvc.value.get().pipe(Effect.catch(() => Effect.succeed(void 0)));
      return {
