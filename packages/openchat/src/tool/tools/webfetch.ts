...
export const WebFetchTool = define(
  'webfetch',
  Effect.gen(function* () {
    const httpOk = HttpClient.filterStatusOk(yield* HttpClient.HttpClient);
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: SchemaTool.Context) =>
        Effect.gen(function* () {
              ...
              () =>
                httpOk.execute(
                  HttpClientRequest.get(params.url).pipe(
                    HttpClientRequest.setHeaders({ ...headers, 'User-Agent': 'openchat' })
                  )
                )
            ),
          ...
          if (MediaUtil.isImageAttachment(mime)) {
