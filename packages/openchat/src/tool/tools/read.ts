...
export const ReadTool = define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn('ReadTool.execute')(
      function* (params: Schema.Schema.Type<typeof Parameters>, ctx: SchemaTool.Context) {
        ...
        const title = path.relative(directory, filepath);
        ...
        const mime = MediaUtil.sniffAttachmentMime(sample, AppFileSystem.mimeType(filepath));
        const isImage = SUPPORTED_IMAGE_MIMES.has(mime);

        if (isImage || MediaUtil.isPdfAttachment(mime)) {
          const bytes = yield* fs.readFile(filepath);
          const msg = MediaUtil.isPdfAttachment(mime)
