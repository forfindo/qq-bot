    ...
    return {
      ...
      execute: (
        ...
        Effect.gen(function* () {
          ...
          const requested = path.isAbsolute(params.path ?? directory)
          ...
          const file = info?.type === 'Directory' ? void 0 : [path.relative(cwd, search)];

          const result = yield* rg.search({
            cwd,
            pattern: params.pattern,
            glob: params.include ? [params.include] : void 0,
            file,
            signal: ctx.abort
          });
          ...
          const times = new Map(
            (yield* Effect.forEach(
              ...
              Effect.fnUntraced(function* (file) {
                const info = yield* fs.stat(file).pipe(Effect.catch(() => Effect.succeed(void 0)));
