...
import { SchemaMessage, SchemaSession, SchemaTool } from '@/schema';
import { deriveSubagentSessionPermission } from '@/agent/subagent-permissions';
import { EffectRunner } from '@/instance';
import DESCRIPTION from './task.md';
import { JsonSchemaTool } from '@/utils';
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn('TaskTool.execute')(function* (
      ...
      const session = params.task_id
        ? yield* sessions
            .get(SchemaSession.SessionID.make(params.task_id))
            .pipe(Effect.catchCause(() => Effect.succeed(void 0)))
        : void 0;
      ...
      const childToolDenies = [
        ...
        ...(next.permission.some(rule => rule.permission === 'task')
          ? []
          : [{ permission: 'task', pattern: '*' as const, action: 'deny' as const }]),
      ...
      const msg = yield* Message.get({ sessionID: ctx.sessionID, messageID: ctx.messageID }).pipe(
      ...
      const runTask = Effect.fn('TaskTool.runTask')(function* () {
        ...
        const result = yield* ops.prompt({
          messageID: SchemaMessage.MessageID.ascending(),
          sessionID: nextSession.id,
          model: {
            modelID: model.modelID,
            providerID: model.providerID
          },
          variant: next.model ? void 0 : variant,
          agent: next.name,
          parts
        });
        ...
        yield* background.wait({ id: jobID }).pipe(
          Effect.flatMap(result => {
            ...
            if (result.info?.status === 'error') {
              return inject('error', result.info.error ?? '');
            }
            return Effect.void;
          }),
      ...
      const runCancel = yield* EffectRunner.make();
    ...
    return {
      description: Flag.EXPERIMENTAL_BACKGROUND_SUBAGENTS
        ? [DESCRIPTION, BACKGROUND_DESCRIPTION].join('\n\n')
        : DESCRIPTION,
      parameters: Parameters,
      jsonSchema: Flag.EXPERIMENTAL_BACKGROUND_SUBAGENTS
        ? void 0
        : JsonSchemaTool.fromSchema(BaseParameters),
