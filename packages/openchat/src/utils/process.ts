...
export const spawn = (cmd: string[], opts: Options = {}): Child => {
  if (cmd.length === 0) {
  ...
  const proc = launch(cmd[0]!, cmd.slice(1), {
    cwd: opts.cwd,
    shell: opts.shell,
    env: opts.env === null ? {} : opts.env ? { ...process.env, ...opts.env } : void 0,
...
export const run = async (cmd: string[], opts: RunOptions = {}): Promise<Result> => {
...
export const stop = async (proc: ChildProcess) => {
  if (proc.exitCode !== null || proc.signalCode !== null) {
    return;
  }
...
export const text = async (cmd: string[], opts: RunOptions = {}): Promise<TextResult> => {
