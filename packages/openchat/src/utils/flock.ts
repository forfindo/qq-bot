...
const root = () => {
  return path.join(Global.Path.state, 'locks');
};
...
async function tryAcquireLockDir(
  ...
  await writeFile(heartbeatPath, '', { flag: 'wx' }).catch(async () => {
    ...
    throw new LockCompromisedError({
      detail: 'Lock acquired but heartbeat already existed (possible compromise).'
    });
  });
  ...
  await writeFile(metaPath, JSON.stringify(meta, null, 2), { flag: 'wx' }).catch(async () => {
    await rm(lockDir, { recursive: true, force: true });
    throw new LockCompromisedError({
      detail: 'Lock acquired but meta.json already existed (possible compromise).'
    });
  });
  ...
  const release = async () => {
    ...
    const current = await readFile(metaPath, 'utf8')
      ...
      .catch(err => {
        ...
        if (errCode === 'ENOENT' || errCode === 'ENOTDIR') {
          throw new LockCompromisedError({
            detail: 'Refusing to release: lock is compromised (metadata missing).'
          });
        }
        if (err instanceof SyntaxError) {
          throw new LockCompromisedError({
            detail: 'Refusing to release: lock is compromised (metadata invalid).'
          });
        }
        throw err;
      });
    ...
    if (current.token !== token) {
      throw new LockCompromisedError({
        detail: 'Refusing to release: lock token mismatch (not the owner).'
      });
    }
...
async function acquireLockDir(
  ...
  while (true) {
    ...
    if (mono() > stop) {
      throw new LockTimeoutError({ key: input.key });
    }
...
export const effect = Effect.fn('Flock.effect')(function* (key: string, input: Options = {}) {
  return yield* Effect.acquireRelease(
    Effect.tryPromise({
      try: signal => acquire(key, { ...input, signal }),
      catch: error => {
        if (error instanceof LockTimeoutError || error instanceof LockCompromisedError) {
          return error;
        }
        throw error;
      }
