...
const snapshot = (job: Active): Info => {
  return {
    ...job.info,
...
export class Service extends Context.Service<Service, Interface>()('@openchat/BackgroundJob') {}
...
export const make = Effect.gen(function* () {
  ...
  const list = Effect.fn('BackgroundJob.list')(function* () {
      ...
      Effect.gen(function* () {
        ...
        if ('scope' in result) {
          yield* fork(
            ...
            result.token,
            0,
            restore(input.run).pipe(Effect.ensuring(Deferred.succeed(tail, void 0)))
          );
        }
        return result.info;
      })
      ...
      Effect.gen(function* () {
        ...
        yield* fork(
          ...
          Deferred.await(result.previous).pipe(
            ...
            Effect.ensuring(Deferred.succeed(result.tail, void 0))
          )
        );
        return true;
      })
  ...
  const promote: Interface['promote'] = Effect.fn('BackgroundJob.promote')(function* (id) {
    const result = yield* SynchronizedRef.modifyEffect(state.jobs, function (jobs) {
      const job = jobs.get(id);
      if (!job || job.info.status !== 'running') {
        return Effect.succeed([{}, jobs] as readonly [PromoteResult, Map<string, Active>]);
      }
      if (job.info.metadata?.background === true) {
        return Effect.succeed([{ info: snapshot(job) }, jobs] as readonly [
          PromoteResult,
          Map<string, Active>
        ]);
      }
    ...
    if (result.onPromote) {
      yield* result.onPromote.pipe(Effect.ignore);
    }
    return result.info;
  });
  ...
  const cancel: Interface['cancel'] = Effect.fn('BackgroundJob.cancel')(function* (id) {
    ...
    if (result.scope) {
      yield* Scope.close(result.scope, Exit.void);
    }
    return result.info;
  });
