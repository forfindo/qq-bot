...
>(
  ...
  return {
    kind: 'layer',
    name: input.service !== void 0 ? input.service.key : input.name,
...
function walk<Result>(
  ...
  const recur = (node: AnyNode): Result => {
    ...
    if (cached !== void 0 || cache.has(target)) {
      return cached!;
    }
...
export const makeTags = tags({
  location: ['global'],
  global: []
});

export const makeGlobalNode = makeTags.make('global');
export const makeLocationNode = makeTags.make('location');
