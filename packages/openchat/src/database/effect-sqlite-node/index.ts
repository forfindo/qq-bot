...
import {
  classifySqliteError as _classifySqliteError,
  SqlError
...
export const TypeId: TypeId = '~@@openchat/database/NodeSqliteClient';
export type TypeId = '~@@openchat/database/NodeSqliteClient';

const classifySqliteError = (
  cause: unknown,
  options: {
  ...
  if (cause instanceof Object) {
    if (
      ...
      Object.hasOwn(cause, 'errcode') &&
      // @ts-ignore
      typeof cause.errcode === 'number'
    ) {
      // @ts-ignore
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      cause.errno = cause.errcode;
    }
  }
  return _classifySqliteError(cause, options);
};
...
export const SqliteClient = Context.Service<SqliteClient>('@@openchat/database/NodeSqliteClient');
