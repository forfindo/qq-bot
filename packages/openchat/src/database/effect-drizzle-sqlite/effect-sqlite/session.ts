import { Effect, Context, Exit, Scope } from 'effect';
...
> {
  ...
  private executeTransactionStatement(
    ...
    return connection.executeUnprepared(query, [], void 0).pipe(Effect.asVoid);
  }
      ...
      Effect.withFiber<A, E | SqlError, R>(fiber => {
        ...
        > =
          connectionOption._tag === 'Some'
            ? Effect.succeed([void 0, connectionOption.value[0]] as const)
            : Scope.make().pipe(
