...
export function orderSelectedFields(
  ...
  return Object.entries(fields).flatMap(([name, field]) => {
    ...
    if (is(field, Table)) {
      return orderSelectedFields(getTableColumnsRuntime(field), path);
    }
...
export function mapResultRow(
  ...
  columns.forEach((column, columnIndex) => {
    const decoder = is(column.field, Column)
      ? column.field
      : is(column.field, SQL)
        ? (column.field as unknown as { decoder: { mapFromDriverValue(value: unknown): unknown } })
            .decoder
    ...
    const value: unknown = rawValue === null ? null : decoder.mapFromDriverValue(rawValue);
    ...
    if (joinsNotNullableMap && is(column.field, Column) && column.path.length === 2 && objectName) {
      const tableName: string = getTableName(getColumnTable(column.field)) as string;
...
export function getTableLikeName(table: SQLiteTable | Subquery | SQLiteViewBase | SQL) {
  ...
  if (is(table, SQLiteViewBase)) {
    return getViewSelectedFieldsRuntime(table).name;
  }
  if (is(table, SQL)) {
    return void 0;
  }
