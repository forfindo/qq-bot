...
> {
  ...
  constructor(
    ...
    protected session: SQLiteEffectSession<TEffectHKT, TRunResult, AnyRelations>,
    ...
    SQLWrapper
{
  ...
  constructor(
    ...
    private effectSession: SQLiteEffectSession<TEffectHKT, TRunResult, AnyRelations>,
  ...
  onConflictDoNothing(config: { target?: IndexColumn | IndexColumn[]; where?: SQL } = {}): this {
    ...
    ${config.where}`
      : sql``;
    this.config.onConflict.push(sql` on conflict
    ${targetSql}
    do
    nothing
    ${whereSql}`);
  ...
  onConflictDoUpdate(config: SQLiteEffectInsertOnConflictDoUpdateConfig<this>): this {
    ...
    this.config.onConflict.push(
      ...
      ${targetWhereSql}
      do
      update set ${setSql}${whereSql}${setWhereSql}`
    );
    return this;
  }
