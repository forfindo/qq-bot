...
> implements PreparedQuery {
  ...
  private jitMapper?: RowsMapper<unknown> | RelationalRowsMapper<unknown>;
  private cacheConfig: WithCacheConfig | undefined;
  private readonly effectExecuteMethod: SQLiteExecuteMethod;
  ...
  constructor(
    ...
    this.cacheConfig =
      cache.strategy() === 'all' && cacheConfig === void 0
  ...
  run(placeholderValues?: Record<string, unknown>): unknown {
  ...
  all(placeholderValues?: Record<string, unknown>): unknown {
    ...
    return this.executeWithCache<T['values'], T['all']>(placeholderValues, 'values', rows =>
  ...
  get(placeholderValues?: Record<string, unknown>): unknown {
    ...
    return this.executeWithCache<T['values'], T['get']>(placeholderValues, 'values', rows =>
      this.mapGetResult(rows)
    );
  }
  ...
  private allRqbV2(placeholderValues?: Record<string, unknown>) {
    return this.executeWithCache<unknown[], T['all']>(placeholderValues, 'all', rows =>
      this.mapAllResult(rows)
    );
  }
  ...
  private getRqbV2(placeholderValues?: Record<string, unknown>) {
    return this.executeWithCache<unknown, T['get'] | undefined>(placeholderValues, 'get', row =>
      row === void 0 ? void 0 : this.mapGetResult(row)
    );
  }
  ...
  private mapCachedResult<A, B>(result: A, mapResult: ((result: A) => B) | undefined) {
    if (!mapResult) {
      return Effect.succeed(result as unknown as B);
    }
    return Effect.try({
  ...
  private queryWithCache<A, E, R, B = A>(
    ...
    return Effect.gen({ self: this }, function* () {
      ...
      if (cacheStrat.type === 'try') {
        ...
        const fromCache: unknown[] | undefined = yield* this.cache.get(
          key,
          tables,
          isTag,
          autoInvalidate
...
> {
  ...
  protected constructor(readonly dialect: SQLiteAsyncDialect) {}
  ...
  all<T = unknown>(query: SQL): unknown {
  ...
  get<T = unknown>(query: SQL): unknown {
  ...
  values<T extends unknown[] = unknown[]>(query: SQL): unknown {
