...
const error = (stderr: string, code: number) => {
...
const parse = (line: string) => {
  return SchemaRipgrep.decodeResult(line).pipe(
    Effect.mapError(cause => new Error('invalid ripgrep output', { cause }))
  );
};

const searchArgs = (input: SchemaRipgrep.SearchInput) => {
...
const filesArgs = (input: SchemaRipgrep.FilesInput) => {
...
const fail = (
  queue: Queue.Queue<string, PlatformError | Error | Cause.Done>,
  err: PlatformError | Error
) => {
  Queue.failCauseUnsafe(queue, Cause.fail(err));
};

const row = (data: SchemaRipgrep.Row): SchemaRipgrep.Row => {
  return {
    ...data,
    path: {
      ...data.path,
...
const aborted = (signal?: AbortSignal) => {
  const err: unknown = signal?.reason;
...
const waitForAbort = (signal?: AbortSignal) => {
  ...
  if (signal.aborted) {
    return Effect.fail(aborted(signal));
  }
...
const raceAbort = <A, E, R>(effect: Effect.Effect<A, E, R>, signal?: AbortSignal) => {
  return signal ? effect.pipe(Effect.raceFirst(waitForAbort(signal))) : effect;
};

const env = () => {
  const env = ProcessUtil.sanitizedProcessEnv();
  delete env.RIPGREP_CONFIG_PATH;
  return env;
};

export interface Interface {
  readonly files: (input: SchemaRipgrep.FilesInput) => Stream.Stream<string, PlatformError | Error>;
  readonly tree: (input: SchemaRipgrep.TreeInput) => Effect.Effect<string, PlatformError | Error>;
  readonly search: (
    input: SchemaRipgrep.SearchInput
  ) => Effect.Effect<SchemaRipgrep.SearchResult, PlatformError | Error>;
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const extract = Effect.fnUntraced(function* (
      ...
      if (!(yield* fs.isFile(extracted))) {
        return yield* Effect.fail(
          new Error(`ripgrep archive did not contain executable: ${extracted}`)
        );
      }
    ...
    const check = Effect.fnUntraced(function* (cwd: string) {
      if (yield* fs.isDir(cwd).pipe(Effect.orDie)) {
        return;
      }
      return yield* Effect.fail(
    ...
    const files = (input: SchemaRipgrep.FilesInput) =>
    ...
    const search = Effect.fn('Ripgrep.search')(function* (input: SchemaRipgrep.SearchInput) {
      yield* check(input.cwd);
          ...
          const [items, stderr, code] = yield* Effect.all(
            [
              Stream.decodeText(handle.stdout).pipe(
                ...
                Stream.filter((item): item is SchemaRipgrep.Match => item.type === 'match'),
    ...
    const tree = Effect.fn('Ripgrep.tree')(function* (input: SchemaRipgrep.TreeInput) {
      // @ts-expect-error TODO: implement log
      log.info('tree', input);
      const list = Array.from(
        yield* files({ cwd: input.cwd, signal: input.signal }).pipe(Stream.runCollect)
      );
      ...
      for (const file of list) {
        ...
        if (parts.includes('.openchat') || parts.includes('openchat')) {
          continue;
        }
        if (parts.length < 2) {
          continue;
