...
import type { NamedError } from '@/utils/error';
import { iife, TypeGuard } from '@/utils';
...
export function delay(attempt: number, error?: SchemaMessage.APIError) {
  if (error) {
...
export function retryable(error: Err, provider: string) {
  ...
  if (SchemaMessage.APIError.isInstance(error)) {
    const status = error.data.statusCode;
    // 5xx errors are transient server failures and should always be retried,
    // even when the provider SDK doesn't explicitly mark them as retryable.
    if (!error.data.isRetryable && !(status !== void 0 && status >= 500)) {
      return void 0;
    }
    ...
    if (error.data.responseBody?.includes('GoUsageLimitError')) {
      ...
      const workspace = str(TypeGuard.isRecord(body?.metadata) ? body?.metadata?.workspace : '');
      const limitName = str(TypeGuard.isRecord(body?.metadata) ? body?.metadata?.limitName : '');
  ...
  const msg = TypeGuard.isRecord(error.data) ? error.data.message : void 0;
  ...
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      ...
      return Effect.gen(function* () {
        const wait = delay(meta.attempt, SchemaMessage.APIError.isInstance(error) ? error : void 0);
