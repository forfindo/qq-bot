...
const toTag = (err: NodeJS.ErrnoException): SystemErrorTag => {
  switch (err.code) {
    case 'ENOENT':
      return 'NotFound';
    case 'EACCES':
...
const killGroup = (
  ...
  if (globalThis.process.platform === 'win32') {
    return Effect.callback<void, PlatformError>(resume => {
...
const setupFds = Effect.fnUntraced(function* (
  ...
  const ins = new Map<number, Sink.Sink<void, Uint8Array, never, PlatformError>>();
  const outs = new Map<number, Stream.Stream<Uint8Array, PlatformError>>();

  for (const x of extra) {
    const node = proc.stdio[x.fd];
    switch (x.config.type) {
      case 'input': {
        let sink: Sink.Sink<void, Uint8Array, never, PlatformError> = Sink.drain;
      ...
      case 'output': {
        let stream: Stream.Stream<Uint8Array, PlatformError> = Stream.empty;
...
const setupStdin = (
  ...
  Effect.suspend(() => {
    let sink: Sink.Sink<void, unknown, never, PlatformError> = Sink.drain;
...
export const make = Effect.gen(function* () {
  ...
  const spawnCommand: (
    command: ChildProcess.Command
  ) => Effect.Effect<ChildProcessHandle, PlatformError, Scope.Scope> = Effect.fnUntraced(
    function* (command) {
      switch (command._tag) {
        case 'StandardCommand': {
