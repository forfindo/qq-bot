...
export const make = <A, E = never>(
  ...
  const idleIfCurrent = () =>
    SynchronizedRef.modify(ref, st => [st._tag === 'Idle' ? idle : Effect.void, st] as const).pipe(
      Effect.flatten
    );
  ...
  const stopShell = (shell: ShellHandle<A, E>) =>
    Effect.gen(function* () {
      if (shell.ready) {
        yield* shell.ready.await.pipe(Effect.exit, Effect.asVoid);
      }
      yield* Deferred.succeed(shell.cancelled, void 0).pipe(Effect.asVoid);
