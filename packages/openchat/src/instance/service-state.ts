...
export interface ServiceState<A, E = never, R = never> {
...
): Effect.Effect<ServiceState<A, E, Exclude<R, Scope.Scope>>, never, R | Scope.Scope> => {
...
export const get = <A, E, R>(self: ServiceState<A, E, R>) =>
  Effect.gen(function* () {
    return yield* ScopedCache.get(self.cache, yield* uid);
  });

export const use = <A, E, R, B>(self: ServiceState<A, E, R>, select: (value: A) => B) =>
  Effect.map(get(self), select);

export const useEffect = <A, E, R, B, E2, R2>(
  self: ServiceState<A, E, R>,
  select: (value: A) => Effect.Effect<B, E2, R2>
) => Effect.flatMap(get(self), select);

export const invalidate = <A, E, R>(self: ServiceState<A, E, R>) =>
  Effect.gen(function* () {
